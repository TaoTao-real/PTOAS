...
void SyncCodegen::CreateSetWaitOpForMultiBuffer(IRRewriter &rewriter,
                                                ...
                                                SyncOperation *sync,
                                                bool beforeInsert) {
  // Generate dynamic event-id selection from loop iv (%iv % 2 ? id1 : id0).
  Value bufferSelected = GetBufferSelected(rewriter, op, sync);
  if (!bufferSelected) {
    // Safety fallback: if we cannot derive a loop-local selector, keep static id.
  ...
  // Multi-buffer sync must use dynamic event-id op to materialize id rotation.
  if (sync->isSyncWaitType()) {
