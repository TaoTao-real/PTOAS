...
void SyncEventIdAllocation::ReallocatedEventId() {
  ...
  // Collect targets first, then mutate sync IR. SetEventId() may insert new
  // loop head/tail syncs and must not run while iterating the same container.
  SmallVector<SyncOperation *> reallocateTargets;
  llvm::DenseSet<SyncOperation *> seenSetSync;
  for (auto &e : syncIR_) {
    for (auto &sync : e->pipeBefore) {
      ...
      if (setSync->uselessSync) {
        continue;
      }
      if (seenSetSync.insert(setSync).second) {
        reallocateTargets.push_back(setSync);
      }
    }
  }
  for (SyncOperation *setSync : reallocateTargets) {
    ClearEventId(setSync);
    SetEventId(setSync);
  }
}

void SyncEventIdAllocation::ClearReallocatedBackwardMatchSync() {
  SmallVector<SyncOperation *> removedBackwardSync;
  for (auto &e : syncIR_) {
    SyncOps newPipeBefore;
    for (auto &sync : e->pipeBefore) {
      bool remove =
          insertedBackwardSync.contains(sync) &&
          reallocatedPipePair.count(ScopePair(sync));
      if (!remove) {
        newPipeBefore.push_back(sync);
      } else {
        removedBackwardSync.push_back(sync);
      }
    }
    e->pipeBefore = newPipeBefore;
  ...
  for (auto *sync : removedBackwardSync) {
    insertedBackwardSync.erase(sync);
  }
}
...
void SyncEventIdAllocation::IgnoreBackHeadAndTailSync() {
  ...
  for (auto *sync : insertedBackwardSync) {
