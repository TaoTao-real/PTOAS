...
### 5.7 借鉴 AscendNPU-IR 的分层方式

AscendNPU-IR 的实现给 PTOAS 一个重要启发：不要把所有内存一致性能力都塞进
InjectSync 这类 pipe 同步 pass。更合理的分工是：
...
以 AscendNPU-IR 为例，`disableDCache` 不是一条硬件能直接识别的 MLIR 指令，
而是标记在 `memref.load` / `LLVM::LoadOp` 上的访存属性。后端再根据该属性生成
`ld_dev` 或 non-cache load。另一方面，`hivm.dcci` 是显式 cache-control op，用于
clean/writeback/invalidate cache。也就是说，AscendNPU-IR 同时使用两种表达方式：

- 能附着在访存指令上的 consistency 语义，用 access policy/attribute 表达。
- 必须独立执行的 consistency action，用显式 op 表达。

PTOAS 应采用类似分层：

- 现有 InsertSync 继续负责 pipe order 和 event id。
- 新增 memory consistency descriptor/pass 负责判断是否需要 `ldg.uncache`、
  `dcci`、`dsb(DSB_DDR)`、`mem_bar` 等动作。
- codegen/lowering 根据 action 类型决定是改写访存 cache policy，还是插入显式指令。

这样可以避免把 `set_flag/wait_flag` 的语义过度扩张，也避免要求所有 OP 都暴露完整的
硬件 cache/fence 细节。
...
collectMemoryAccessDescs(op):
  if op implements MemoryConsistencyOpInterface:
