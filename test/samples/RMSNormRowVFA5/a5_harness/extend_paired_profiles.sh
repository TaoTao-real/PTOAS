...
experiment_dir=$1
source_root=$2
rows_list=${ROWS_LIST:?ROWS_LIST is required}
...
for rows in $rows_list; do
  for variant in D DL; do
    fixture="$experiment_dir/inputs/fixture/n${rows}/${dataset}"
    warmup="$experiment_dir/work/runs/n${rows}-${variant}-extension-warmup"
    mkdir -p "$warmup"
    cp "$fixture/x.bin" "$warmup/x.bin"
    cp "$fixture/gamma.bin" "$warmup/gamma.bin"
    cp "$fixture/y.init.bin" "$warmup/y.bin"
    (cd "$warmup" && timeout 120 "$experiment_dir/work/build/n${rows}/${variant}")
