...
int main() {
  constexpr size_t kElements = 8 * 128;
  ...
  ACL_CHECK(aclrtMallocHost(reinterpret_cast<void **>(&srcHost), kBytes));
  ACL_CHECK(aclrtMallocHost(reinterpret_cast<void **>(&dstHost), kBytes));
  ACL_CHECK(aclrtMalloc(reinterpret_cast<void **>(&srcDevice), kBytes,
                       ACL_MEM_MALLOC_HUGE_FIRST));
  ACL_CHECK(aclrtMalloc(reinterpret_cast<void **>(&dstDevice), kBytes,
                       ACL_MEM_MALLOC_HUGE_FIRST));

  if (!ReadFile("./v1.bin", inputBytes, srcHost, kBytes)) {
  ...
  LaunchVmiTilelibLogicalRowRmsnorm(srcDevice, dstDevice, stream);
